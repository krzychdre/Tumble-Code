...
export class SwitchModeTool extends BaseTool<"switch_mode"> {
	...
	async execute(params: SwitchModeParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			// Check if already in requested mode. The task's own mode: provider state holds the
			// focused task's mode, and a background subagent or a delegated child may differ.
