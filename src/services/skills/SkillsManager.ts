...
`
	...
	> {
		...
		//
		// RooDirectoryResolver lists the directories in processing order:
		// ~/.agents, <cwd>/.agents, ~/.roo, <cwd>/.roo (so .roo wins over .agents).
		const dirs = await RooDirectoryResolver.list(provider?.cwd, {
			kind: "skills",
			modes: await this.getAvailableModes(),
		})

		return dirs.map(({ path: dir, source, mode }) => ({
			dir,
	...
	private async setupFileWatchers(): Promise<void> {
		...
		// Watch the same directories discovery reads.
		for (const { dir } of await this.getSkillsDirectories()) {
