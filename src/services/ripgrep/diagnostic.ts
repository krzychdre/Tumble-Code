...
export async function getRipgrepDiagnostic(vscodeAppRoot: string): Promise<string> {
	...
	const lines: string[] = [
		`Tumble Code Ripgrep Diagnostic (${new Date().toISOString()})`,
...
/**
 * Registers the `tumble-code.showRipgrepDiagnostic` command. Thin wrapper —
...
export function registerRipgrepDiagnosticCommand(): vscode.Disposable {
	const channel = vscode.window.createOutputChannel("Tumble Code Ripgrep Diagnostic")
	const command = vscode.commands.registerCommand(getCommand("showRipgrepDiagnostic"), async () => {
		...
		await vscode.window.showInformationMessage("Tumble Code: ripgrep diagnostic copied to clipboard.")
	})
