...
<p align="center">
  <!-- <em>快速获取帮助 → <a href="https://discord.gg/tumblecode">加入 Discord</a> • 偏好异步？→ <a href="https://www.reddit.com/r/TumbleCode/">加入 r/Tumble Code</a></em> -->
</p>
...
> ### Tumble Code 插件不会消失。
>
> 你可能已经看到[最近的公告](https://x.com/mattrubens/status/2046636598859559114)：Tumble Code 已突破 300 万次安装，原团队正全力投入 Roomote。我们知道这个消息让许多人感到难受。这个插件对我们和你都意义重大，你的心声我们听到了。
>
> 好消息是：**一支社区团队已挺身而出，继续推进 Tumble Code**，我们正与他们协作进行正式交接，让你所依赖的插件继续得到维护和改进。
...
了解更多：[使用模式](https://docs.tumblecode.dev/basic-usage/using-modes) • [自定义模式](https://docs.tumblecode.dev/advanced-usage/custom-modes)
...
<p align="center">
<a href="https://docs.tumblecode.dev/tutorial-videos">更多快速教程和功能视频...</a>
</p>
...
有几种方法可以运行 Tumble Code 扩展程序：

### 开发模式（F5）

对于积极开发，请使用 VSCode 的内置调试功能：

在 VSCode 中按 `F5`（或转到 **Run** → **Start Debugging**）。这将在运行 Tumble Code 扩展程序的新 VSCode 窗口中打开。
...
1.  首先，构建 VSIX 包：
    ...
    pnpm vsix
    ```
2.  将在 `bin/` 目录中生成一个 `.vsix` 文件（例如，`bin/tumble-code-<version>.vsix`）。
...
**请注意**，Tumble Code, Inc. **不**对与 Tumble Code 相关的任何代码、模型或其他工具、任何相关的第三方工具或任何由此产生的输出作出任何陈述或保证。您承担使用任何此类工具或输出的**所有风险**；此类工具均按**“原样”**和**“可用”**的基础提供。此类风险可能包括但不限于知识产权侵权、网络漏洞或攻击、偏见、不准确、错误、缺陷、病毒、停机、财产损失或损害和/或人身伤害。您对自己使用任何此类工具或输出负全部责任（包括但不限于其合法性、适当性和结果）。
...
<!-- **享受 Tumble Code！** 无论您是让它保持短绳还是让它自主漫游，我们都迫不及待地想看看您会构建什么。如果您有问题或功能想法，请访问我们的 [Reddit 社区](https://www.reddit.com/r/TumbleCode/)或 [Discord](https://discord.gg/tumblecode)。编码愉快！ -->
