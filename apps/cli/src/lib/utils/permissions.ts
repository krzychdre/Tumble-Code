...
export const PERMISSIONS_COMMAND_USAGE = "Usage: /permissions [ask|allow]"
...
export type PermissionArgumentResult = { success: true; mode: PermissionMode } | { success: false; error: string }
...
export function resolvePermissionArgument(argument: string, currentMode: PermissionMode): PermissionArgumentResult {
	...
	if (tokens.length === 0) {
		return { success: true, mode: currentMode === "allow" ? "ask" : "allow" }
	}
