...
# Task.ts                  ~800
# TaskHistory.ts           ~250
# TaskTokenTracking.ts     ~120
# TaskAskSay.ts            ~350
# TaskSubtasks.ts          ~100
# TaskContextManager.ts    ~250
# TaskStreamProcessor.ts   ~400
# StreamProcessorTypes.ts   ~30
# TaskLifecycle.ts         ~400
# TaskApiLoop.ts           ~600
...
## Summary: File Structure After Refactoring

```
src/core/task/
├── Task.ts                      ~800 lines  (coordinator + constructor + delegation)
...
├── StreamProcessorTypes.ts       ~30 lines  (shared types for stream processing)
...
├── TaskSubtasks.ts              ~100 lines  (subtask delegation & resumption)
...
**No file exceeds 600 lines.** Each file has a single clear responsibility. The coordinator `Task.ts` only contains:
